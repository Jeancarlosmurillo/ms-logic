...
export default class LotsController {
  public async find({ request, params }: HttpContextContract) {
    if (params.id) {
      ...
      await theLot.load("route", (Route) => {
        Route.preload("contract");
      });
      await theLot.load("products", (Product) => {
        Product.preload("categoryProduct", (CategoryProduct) => {
          CategoryProduct.preload("category");
        });
      });
