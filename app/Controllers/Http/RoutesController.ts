...
/*export default class RoutesController {
    ...
    public async find({ request, params }: HttpContextContract) {
        if (params.id) {
            ...
            await theRoute.load("contract",(Contract) => {Contract.preload('customer', (Customer) => {
                Customer.preload('NaturalPerson', (User) => {User.preload('user')})
                Customer.preload('companies', (Company) => Company.preload('naturalperson', (User) => {User.preload('user')}))})});
            ...
            if ("page" in data && "per_page" in data) {
                ...
                return await Route.query().preload('contract',(Contract) => {Contract.preload('customer', (Customer) => {
                    Customer.preload('NaturalPerson')
                    Customer.preload('companies', (Company) => Company.preload('naturalperson'))})}                )
                .preload('vehicle').paginate(page, perPage)
            } else {
