...
export default class ShiftValidator {
  ...
  public schema = schema.create({
    type_shift: schema.enum(['diurno', 'nocturno', 'turno extra']), // Enum para tipos de turnos
    status: schema.string({}, [
      rules.maxLength(15), // Limita el estado a 15 caracteres
    ]),
    start_date: schema.date({ format: 'yyyy-MM-dd HH:mm:ss' }, [
      rules.beforeField('end_date'), // Asegura que la fecha de inicio sea anterior a la fecha de fin
    ]),
